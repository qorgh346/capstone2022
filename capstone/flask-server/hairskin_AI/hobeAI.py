...
class HobeAI():
    def __init__(self,pretrained_path=None):
        ...
        self.main_transforms = transforms.Compose([
                                        transforms.Resize([int(255), int(255)]),
                                        transforms.ToTensor(),
    ...
    def get_Network(self,type):
        Network_List = {'moblievitNet': kgu_Network(feature_extract=True, device='cpu', num_class=4),
                             'efficientNet': EfficientNet.from_pretrained('efficientnet-b1', num_classes=4)
                             }
    ...
    def process(self,image=None):
        ...
        for idx,image in enumerate(image_list):
            ...
            for name,model in self.get_model(type='moblievitNet',name='vit_model_s').items():
                predict = model(img)
